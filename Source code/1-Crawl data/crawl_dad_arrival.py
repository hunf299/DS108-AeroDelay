...
ARRIVAL_DIR = BRONZE / "Arrival" / "dad_flights_arrival_bronze_layer1.csv"
...
def sort_final_csv(file_path):
    try:
        ...
        # Sắp xếp: Crawl_Date tăng dần, Scheduled_Time tăng dần (00:00 -> 23:59)
...
def crawl_dad_historical_flights_edge(date_chunk, thread_id):
    try:
        ...
        for target_date_str in date_chunk:
            ...
            cols_arrival = ["Crawl_Date", "Scheduled_Time", "Actual_Time", "Origin", "IATA", "Airline", "Flight_No",
                            "Terminal", "Belt", "Status"]

            for row in rows:
                cols = row.find_all('td')

                if len(cols) >= 8:  # Bảng Arrival của Đà Nẵng có ít cột hơn Departure
                    try:
                        ...
                        # --- 2. Xử lý Cột 1: Điểm xuất phát (Origin) & IATA ---
                        origin_full = cols[1].text.strip()
                        match = re.search(r'(.+?)\s*\((.*?)\)', origin_full)
                        if match:
                            clean_origin = match.group(1).strip()
                            iata_code = match.group(2).strip()
                        else:
                            clean_origin = origin_full
                            iata_code = ""

                        # --- 3. Xử lý Cột 3: Tên Hãng ---
                        airline_name = cols[3].text.strip()
                        ...
                        # --- 6. Xử lý Cột 6: Băng chuyền (Belt) ---
                        clean_belt = cols[6].text.strip()

                        # --- 7. Xử lý Cột 7: Trạng thái (Status) ---
                        clean_status = cols[7].text.strip()
                        ...
                        append_to_csv(flight_info, ARRIVAL_DIR, cols_arrival)
